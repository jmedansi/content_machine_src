#!/usr/bin/env python3
# main.py — Point d'entrée LinkedIn (orchestration complète)
import sys
import io
import json
import logging
...
from agents.agent_writer import write_validated_topics, write_linkedin_post
from agents.agent_publisher import post_linkedin
from agents.agent_memory import log_published
from agents.scheduler.agent import process_single_post, run_pipeline, publish_pending_posts
...
def get_account_id_from_db():
    """Cherche le premier compte LinkedIn actif en DB."""
    try:
...
def run_write(account_id=1):
    ...
    try:
        ...
        validated = [t for t in topics if t.get("validated", False)]
        if not validated:
        ...
        for topic in validated:
            folder = write_linkedin_post(topic, indication=None, account_id=account_id)
            if folder:
                folders.append(folder)
        
        print(f"\n✅ {len(folders)} post(s) rédigé(s) pour le compte {account_id}.")
...
def run_publish(account_id=1):
    """Étape 3 — Publier les posts rédigés non publiés d'un compte"""
    if not config_manager.validate_config():
        return

    print(f"\n🚀 Publication des posts - Compte {account_id}\n")
    ...
    if not folders_to_publish:
        print(f"ℹ️ Aucun post en attente de publication.")
        return

    print(f"📦 {len(folders_to_publish)} post(s) trouvé(s).\n")
    ...
    for folder in folders_to_publish:
        try:
            ...
            success = post_linkedin(str(folder), account_id=account_id)
...
def run_orchestrated(account_id=1, topics_count=5, publish=False):
    ...
    print(f"\n{'='*60}")
    print(f"🔗 Pipeline LinkedIn Orchestrée")
    ...
    try:
        ...
        print(f"   Posts générés: {success_count}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="LinkedIn Machine - Génération & Publication de posts",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Exemples:
  python main.py generate -a 1           # Générer topics pour compte 1
  python main.py write -a 1              # Rédiger posts validés
  python main.py publish -a 1            # Publier posts en attente
  python main.py orchestrated -a 1 -p    # Pipeline complète avec publication
    ...
    parser.add_argument(
        ...
        default=None,
        dest='account_id',
        help='ID du compte LinkedIn (défaut: 1 ou premier compte DB)'
    )
    ...
    try:
        ...
        if args.command == 'generate':
            ...
            print(f"📋 Posts du compte {account_id}:")
            content_dir = Path("accounts") / str(account_id) / "content"
