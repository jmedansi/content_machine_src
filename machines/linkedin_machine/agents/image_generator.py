...
# Import du module de nettoyage de filigrane
try:
    from watermark_eraser_tool import erase_gemini_watermark
except ImportError:
    # Fallback si absent (normalement déjà créé)
...
async def main():
    ...
    try:
        ...
        prompt = p_path.read_text(encoding="utf-8").strip() if p_path.exists() else "A majestic lion"
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) > 1:
        with open(PROMPT_FILE, "w", encoding="utf-8") as f: f.write(sys.argv[1])
    asyncio.run(main())
