# -*- coding: utf-8 -*-
"""
publisher/publish.py — Publication de ressources CTA sur GitHub Pages.
"""
import os
import re
import logging
...
GITHUB_REPO = os.getenv("GITHUB_REPO", "jmedansi/incidenx-audit")
...
def publish_cta(slug: str, title: str, content: str, theme: str = "default") -> dict:
    """
    ...
    Args:
        ...
        theme: Thème visuel optionnel ("default", "blue", "red", "purple")
    
    Returns:
        dict: {"success": True, "url": "https://...", "slug": "..."}
    ...
    try:
        ...
        if _commit_file(f"{slug}/index.html", html, f"Deploy: {slug}"):
            url = f"https://{AUDIT_DOMAIN}/{slug}/"
            logger.info(f"Published: {url}")
            return {"success": True, "url": url, "slug": slug}
        else:
