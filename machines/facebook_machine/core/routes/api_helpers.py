...
def _get_platform_dir(platform: str) -> Path:
    ...
    bases = {
        "facebook": Config.BASE_DIR / "machines" / "facebook_machine",
        "linkedin": Config.BASE_DIR / "machines" / "linkedin_machine",
        "twitter":  Config.BASE_DIR / "machines" / "twitter_machine",
        "instagram": Config.BASE_DIR / "machines" / "facebook_machine",
    }
    return bases.get(platform, Config.BASE_DIR)

def _get_content_dir(platform: str, account_id: int = None) -> Path:
    """Résout le répertoire de contenu pour une plateforme/compte."""
    base = _get_platform_dir(platform)
    if account_id:
        return base / "accounts" / str(account_id) / "content"
    return base / "content"
