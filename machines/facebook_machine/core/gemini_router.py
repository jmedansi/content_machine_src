    ...
    for current_model in models_to_try:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                logger.error(f"[GEMINI_ROUTER] 400 Bad Request ({current_model}): {resp.text[:300]}")
                continue

            else:
                logger.error(f"[GEMINI_ROUTER] HTTP {resp.status_code} ({current_model}): {resp.text[:200]}")
                continue
