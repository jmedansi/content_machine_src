...
try:
    from core.paths import GITHUB_REPO, GITHUB_BRANCH, CHROME_DEBUG_PORT, GEMINI_MAX_WAIT
except ImportError:
...
class GeminiEngine:
    ...
    async def _run_once(self, prompt, local_image=None, mode="generate"):
        ...
        async with async_playwright() as p:
            ...
            # Gestion de l'overlay "Pick a style" si présent
            try:
                style_overlay = await page.query_selector('div:has-text("Pick a style")')
                if style_overlay:
                    print("[INFO] Overlay 'Pick a style' détecté. Tentative de fermeture ou contournement...")
                    # On essaie d'appuyer sur Echap ou de cliquer ailleurs
            ...
            s_btn = await page.query_selector('button[aria-label*="Send"], button[aria-label*="Envoyer"], button[aria-label*="message"]')
            if s_btn:
                await s_btn.click()
                submitted = True
            ...
            print("[INFO] Recherche de l'image pour téléchargement (15 essais)...")
            await asyncio.sleep(5)
            img_data = None

            for retry in range(15):
                try:
                    # Sélecteur plus large pour l'image (Gemini encapsule souvent dans un bouton ou div spécifique)
                    img_container = await page.query_selector('button:has(img[src*="blob"]), .image-button, [data-test-id*="image"]')
                    if img_container:
                        print(f"[INFO] Conteneur image trouvé (essai {retry+1})...")
                        await img_container.hover(force=True)
                        await page.wait_for_timeout(2000)
                        
                        # Recherche du bouton de téléchargement par icône ou label
                        d_btn = await page.query_selector('button[aria-label*="Download"], button[aria-label*="Télécharger"], button:has(svg path[d*="M5 20"])')
                        if d_btn:
                            await d_btn.click()
                    ...
                    print(f"[DEBUG] Erreur pendant le scan image: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reference-image", default=None, help="Reference image path for modify mode")
    parser.add_argument("--mode", "-m", default="generate", choices=["generate", "modify"], help="Generation mode")
    parser.add_argument("prompt", nargs="?", default="")
    args = parser.parse_args()
    
    engine = GeminiEngine()
    local_image = args.reference_image or args.image
    mode = args.mode
    if mode == "modify" and not local_image:
        raise RuntimeError("Mode modify requires --reference-image <path> or --image <path>")
    
    print(f"[INFO] CLI mode={mode} reference_image={local_image}")
