...
_LINKEDIN_DIR = _BASE_DIR / "machines" / "linkedin_machine"
_TWITTER_DIR = _BASE_DIR / "machines" / "twitter_machine"


def _get_platform_dir(platform: str) -> Path:
    platform_key = (platform or "").lower()
    if platform_key == "linkedin":
        return _LINKEDIN_DIR
...
def _get_content_dir(platform: str, account_id: int = None) -> Path:
    base_dir = _get_platform_dir(platform)
    if account_id:
        return base_dir / "accounts" / str(account_id) / "content"
...
async def linkedin_pending(account_id: int = None):
    """Posts en attente de validation LinkedIn."""
    content_dir = _get_content_dir("linkedin", account_id)
    content_dir.mkdir(parents=True, exist_ok=True)
    return {"count": len(_get_pending_posts(content_dir)), "posts": _get_pending_posts(content_dir)}


@router.get("/linkedin/published")
async def linkedin_published(account_id: int = None):
    """Posts publishés LinkedIn."""
    content_dir = _get_content_dir("linkedin", account_id)
    content_dir.mkdir(parents=True, exist_ok=True)
    return {"count": len(_get_published_posts(content_dir)), "posts": _get_published_posts(content_dir)}
...
async def linkedin_approve(body: dict):
    ...
    meta_file = _get_content_dir("linkedin", account_id) / folder / "meta.json"
...
async def linkedin_publish(body: dict):
    ...
    try:
        base_dir = _get_platform_dir("linkedin")
        sys.path.insert(0, str(base_dir))
        from agents.agent_publisher import post_linkedin
        
        if carousel_id:
            if account_id:
                carousel_dir = base_dir / "accounts" / str(account_id) / "carousel"
            else:
        ...
        if folder:
            content_dir = _get_content_dir("linkedin", account_id) / folder
...
async def twitter_pending(account_id: int = None):
    """Posts en attente de validation Twitter."""
    content_dir = _get_content_dir("twitter", account_id)
    content_dir.mkdir(parents=True, exist_ok=True)
    return {"count": len(_get_pending_posts(content_dir)), "posts": _get_pending_posts(content_dir)}


@router.get("/twitter/published")
async def twitter_published(account_id: int = None):
...
async def twitter_approve(body: dict):
    ...
    meta_file = _get_content_dir("twitter", account_id) / folder / "meta.json"
    if not meta_file.exists():
        return {"success": False, "error": "Dossier introuvable"}
...
async def twitter_thread(account_id: int = None):
...
- Dernier tweet avec CTA (follow, like, RT)"""
    ...
    result = await call_groq(prompt, model=Config.TWITTER_LLM_MODEL)
...
async def twitter_publish(body: dict):
    ...
    try:
        base_dir = _get_platform_dir("twitter")
        sys.path.insert(0, str(base_dir))
        from agents.agent_publisher import post_tweet, post_thread
        
        if thread_id:
            content_dir = _get_content_dir("twitter", account_id) / thread_id
            result = post_thread(str(content_dir))
        elif folder:
            content_dir = _get_content_dir("twitter", account_id) / folder
...
async def twitter_generate(account_id: int = None):
    ...
    def _run():
        try:
            ...
            tweet = call_groq(prompt, model=Config.TWITTER_LLM_MODEL, temperature=0.85, max_tokens=280)
