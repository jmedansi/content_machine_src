...
def _preview_jpeg(html: str, output_jpeg: str) -> bool:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        future = executor.submit(_preview_jpeg_sync, html, output_jpeg)
        return future.result()


def _preview_jpeg_sync(html: str, output_jpeg: str) -> bool:
    from playwright.sync_api import sync_playwright
...
if __name__ == "__main__":
  ...
  os.makedirs("d:/Content_Machine/machines/facebook_machine/temp", exist_ok=True)
    ...
    base = "d:/Content_Machine/machines/facebook_machine/temp"
