...
def generate_and_render(
    ...
    if progress_callback: progress_callback(10, "Génération du script AV...")
    script = generate_av_script(topic, persona=persona)
    if not script:
        print("[DIRECTOR] Echec generation script")
        if progress_callback: progress_callback(10, "Échec: Impossible de générer le script")
        return False
    ...
    if progress_callback: progress_callback(30, f"Génération voix off ({len(script)} segments)...")
    ...
    if progress_callback: progress_callback(40, f"Préparation du rendu ({len(script_with_tts)} scènes)...")
    ...
    for i, scene in enumerate(script_with_tts):
        ...
        # Calcul de la progression (de 40% à 85%)
        pct = 40 + int(45 * (i + 1) / len(script_with_tts))
        
        if ok:
            scene_paths.append(scene_out)
            print(f"  OK  (preview: previews/slide_{i:02d}_{stype}.jpg)")
            if progress_callback: progress_callback(pct, f"Rendu scène {i+1}/{len(script_with_tts)} terminé")
        else:
            print(f"  FAIL — slide ignoree")
            if progress_callback: progress_callback(pct, f"Échec rendu scène {i+1}/{len(script_with_tts)}")
    ...
    # 5. Assemblage final
    if progress_callback: progress_callback(85, f"Assemblage final des {len(scene_paths)} scènes...")
...
if __name__ == "__main__":
    generate_and_render(
        ...
        output_path="d:/Content_Machine/machines/facebook_machine/temp/cinema_av_demo.mp4",
    )
